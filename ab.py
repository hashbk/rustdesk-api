...
def send_heartbeat(url, id, uuid, ver=1, modified_at=0):
    """Send device heartbeat to register the device"""
    ...
    response = requests.post(f"{url}/api/heartbeat", headers=headers, json=payload)

    # Heartbeat returns 201, check for both 200 and 201
